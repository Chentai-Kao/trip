...
def create_chain_csp(n):
    ...
    for i in range(len(variables)-1):
        csp.add_binary_potential(variables[i], variables[i+1], lambda x, y : x != y)
...
def create_nqueens_csp(n = 8):
    ...
    for i in range(0, len(variables)):
        for j in range(i+1, len(variables)):
            csp.add_binary_potential(variables[i], variables[j], lambda x, y:x != y and abs(x-y)!=abs(i-j) )
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            # BEGIN_YOUR_CODE (around 10 lines of code expected)
            min_choices = sys.maxint
            min_index = -1;
            for var_idx in range(len(assignment)):
                if assignment[var_idx] == None:
                    possible = list(self.domains[var_idx])
                    for var_idx2 in range(len(assignment)):
                        if assignment[var_idx2] != None:
                            if assignment[var_idx2] in possible:
                                possible.remove(assignment[var_idx2])
                            diff = abs(var_idx-var_idx2)
                            if int(assignment[var_idx2])+diff in possible:
                                possible.remove(int(assignment[var_idx2])+diff)
                            if int(assignment[var_idx2])-diff in possible:
                                possible.remove(int(assignment[var_idx2])-diff)
    ...
    def arc_consistency_check(self, var):
        ...
        queue = [var]
        while len(queue)!=0:
            # print "in while, len of queue = ", len(queue)
            cur_var = queue.pop(0)
            # print "cur_var: ", cur_var
            for var2 in self.csp.get_neighbor_vars(cur_var):
                # print "var2: ", var2
                old_length = len(self.domains[var2])

                # print "before var2 domain: ", self.domains[var2]
                new_domain = []
                for val2 in self.domains[var2]:
                    # print [1 for x in self.domains[cur_var] if self.csp.binaryPotentials[cur_var][var2][x][val2]]
                    if sum([1 for x in self.domains[cur_var] if self.csp.binaryPotentials[cur_var][var2][x][val2]]) > 0:
                        new_domain.append(val2)
                self.domains[var2] = new_domain
                # self.domains[var2] = [x for x in self.domains[var2] if self.csp.binaryPotentials[cur_var][var2][self.domains[cur_var][0]][x] != 0]
                # print "after: var2 domain: ", self.domains[var2]
                new_length = len(self.domains[var2])
                if new_length != old_length and var2 not in queue:
                    queue.append(var2)
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    csp.add_variable(result, [x for x in range(maxSum+1)])
    ...
    for i, X_i in enumerate(variables):
        ...
        # use systematic naming to avoid naming collision
        A_i = ('sum', name, i)
        csp.add_variable(A_i, [(x, y) for x in range(maxSum+1) for y in range(x, maxSum+1)])
        ...
        if i == 0:
            ...
            def potential(b1, b2):
                return b1[1] == b2[0]
            csp.add_binary_potential(('sum', name, i - 1), A_i, potential)

    # consistency between A_n and result
    # hacky: reuse A_i because of python's loose scope
    csp.add_binary_potential(A_i, result, lambda val, res: res == min(val[1], maxSum))
    return result
    # raise Exception("Not implemented yet")
    # END_YOUR_CODE
...
class SchedulingCSPConstructor():
    ...
    def add_quarter_constraints(self, csp):
        ...
        for req in self.profile.requests:
            for quarter in self.profile.quarters:
                if len(req.quarters) != 0 and quarter not in req.quarters:
                    csp.add_unary_potential((req, quarter), lambda cid: cid is None)
                    # print quarter, req.quarters, quarter in req.quarters
    ...
    def add_request_weights(self, csp):
        ...
        for req in self.profile.requests:
            for quarter in self.profile.quarters:
                if req.weight > 1:
                    csp.add_unary_potential((req, quarter), \
                        lambda cid: req.weight if cid != None else 1)
        # raise Exception("Not implemented yet")
        # END_YOUR_CODE
    ...
    def add_unit_constraints(self, csp):
        ...
        for req in self.profile.requests:
            for cid in req.cids:
                for quarter in self.profile.quarters:
                    csp.add_variable((cid, quarter), [x for x in \
                        range(self.bulletin.courses[cid].minUnits, self.bulletin.courses[cid].maxUnits+1)] + [0])
        for req in self.profile.requests:
            for cid in req.cids:
                for quarter in self.profile.quarters:
                    def potential(x,y):
                        if x == cid:  
                            ...
                            return y==0
                    csp.add_binary_potential((req, quarter), (cid, quarter), potential)    

        for quarter in self.profile.quarters:
            var = []
            for req in self.profile.requests:
                for cid in req.cids:
                    var.append((cid, quarter))
            sumVar = get_sum_variable(csp, quarter, var, self.profile.maxUnits)
            csp.add_unary_potential(sumVar, lambda s: s >= self.profile.minUnits and s <= self.profile.maxUnits)
